...
const ICON = {
  ...
  // A tape measure, for the product page's Size Chart button. Ticks hang from
  // the top edge so it still reads as a ruler at 20px, where the body of the
  // rectangle is only about six pixels tall.
  ruler: '<rect x="2.5" y="8" width="19" height="8" rx="2"></rect><path d="M7 8v3M11 8v4M15 8v3M19 8v4"></path>',
...
Order on WhatsApp</a>`;
  ...
  // The size chart, next to the order button. The address is a settings field
  // the owner pastes — the PDF is hosted on ImageKit like every photo, not
  // committed here — so it can be blank, and it can be junk.
  //
  // Both cases render NO button rather than a dead one. safeHref collapses
  // anything outside its scheme allowlist to "#", and an inert pill labelled
  // "Size Chart" is worse than no pill: it reads as available and does
  // nothing. Blank is a legitimate state, not a mistake — it is how the owner
  // turns the button off site-wide.
  //
...
${svg(ICON.ruler, { size: 20, stroke: "currentColor", width: 2 })}
Size Chart</a>`;
...
`;
  ...
  // Fabric doesn't apply to hair accessories (a beaded tiara isn't a fabric
  // piece), so any accessory subcategory — girls, boys, or future — skips
  // straight to Occasion, Fit and Care. Matched on the subcategory id rather
  // than its display name, so renaming the tab (as happened once already)
